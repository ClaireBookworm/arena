...
class RandomAgent(Agent):
    ...
    def get_action(self) -> ActType:
        # YOUR CODE HERE
        action = np.random.default_rng().integers(low=0, high=self.num_arms, size=1).item()
        assert type(action) == int, "Action should be an integer"
        return action
...
class RewardAveraging(Agent):
    ...
    def reset(self, seed: int):
        ...
        self.Q = [float(self.optimism)]*self.num_arms
...
class UCBActionSelection(Agent):
    ...
    def get_action(self):
        # YOUR CODE HERE
        t = np.sum(self.N)
        action = np.argmax(self.Q + self.c*((np.log(t)/self.N)**0.5))
        return action
    ...
    def reset(self, seed: int):
        # YOUR CODE HERE
        super().reset(seed)
        self.Q = [0.0] * self.num_arms
        self.N = [0]* self.num_arms
...
#%%
# Section 3: Tabular RL & Policy Improvement
class Environment:
...
class Toy(Environment):
    def dynamics(self, state: int, action: int):
        ...
        Given dynamics function, Environment class will construct the T and R tensors.
        '''
        (SL, S0, SR) = (0, 1, 2)
        LEFT = 0
        num_states = 3
...
def policy_eval_numerical(env: Environment, pi: Arr, gamma=0.99, eps=1e-8, max_iterations=10_000) -> Arr:
    ...
    '''
    # idx = np.arange(env.num_states)
    # # states = env.num_states,
    # actions = pi
    # transition = env.T[idx, actions, :]
    # rewards = env.R[idx, actions, :]
    V = np.zeros(env.num_states)
    n = 0
    s_id = range(0, env.num_states)
    while n <= max_iterations:
        V_new = np.sum(env.T[s_id, pi] * (env.R[s_id, pi] + (gamma * V)), axis=-1)
        #V_new = einops.einsum(transition, rewards + (gamma * V), "states  s_prime, states s_prime -> states ")
        if abs(V_new - V).max() < eps:
            return V_new
        V = V_new
        n += 1 
    return V
...
def policy_eval_exact(env: Environment, pi: Arr, gamma=0.99) -> Arr:
    ...
    '''
    # # v = np.zeros(env.num_states)
    s_id = np.arange(env.num_states)
    P = env.T[s_id, pi, :]
    R = env.R[s_id, pi, :]
    r = einops.einsum(P, R, "s s_next, s s_next -> s" )
    # # print(r.shape)
    # # identity = np.eye(env.num_states)
    # # v = einops.einsum(np.linalg.inv(identity - gamma * P), r, "s s_next, s -> s")
    mat = np.eye(env.num_states) - gamma * P
    v = np.linalg.solve(mat , r)
    return v
...
def policy_improvement(env: Environment, V: Arr, gamma=0.99) -> Arr:
    ...
    '''
    s_id = np.arange(env.num_states)
    # Q (numstates, numaction)
    Q = einops.einsum(env.T, env.R + gamma * V, "num_states num_actions s_next, num_states num_actions s_next -> num_states num_actions")
    return np.argmax(Q, axis=-1)


tests.test_policy_improvement(policy_improvement)
# %%
def find_optimal_policy(env: Environment, gamma=0.99, max_iterations=10000):
    '''
    Args:
        env: environment
    Outputs:
    ...
    while n <= max_iterations:
        ...
        new_value = policy_eval_exact(env, new_pi, gamma)
        # assert all(new_value >= V)
        if all(new_pi == pi): 
            return new_pi
        pi = new_pi
        V = new_value
